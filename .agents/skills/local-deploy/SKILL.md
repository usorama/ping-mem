...
bun run build
```
**WHY FIRST**: MCP stdio (configured in ~/.Codex/mcp.json) runs from the HOST filesystem's `dist/mcp/cli.js`, not from inside the Docker container. If dist/ isn't rebuilt before deploy, all Codex sessions get stale tools — new tools won't appear, code fixes won't take effect for MCP callers.
