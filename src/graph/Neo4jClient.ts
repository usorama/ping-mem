...
export class Neo4jClient {
   ...
   * halfOpenAfterMs elapses. resetPolicies() resets internal circuit state to
   * CLOSED so neo4j_roundtrip in the warm-up succeeds immediately.
   *
   * The outer ServicePolicy object identity is preserved — in-flight async
   * operations holding references will use the refreshed internals on their
   * next execute() call, and onStateChange handlers remain registered.
   *
   * Not for use in normal operation — rely on cockatiel half-open self-recovery
   * instead.
   */
  resetPolicies(): void {
    // Reset internal circuit state without replacing the outer ServicePolicy objects.
    // This preserves object identity so any in-flight async operations still hold a
    // reference to the same ServicePolicy and their feedback is not lost.
    // onStateChange handlers registered in the constructor are also preserved.
    this.servicePolicy.reset();
    this.writePolicy.reset();
  }
