    ...
    .map((row) => {
      const cells = columns
        .map((col) => {
          ...
          return `<td${col.class ? ` class="${escapeHtml(col.class)}"` : ""}>${value}</td>`;
        })
        .join("");
      ...
      if (options?.hxGet) {
        attrs.push(`hx-get="${escapeHtml(options.hxGet(row))}"`);
        attrs.push(`hx-target="${escapeHtml(options.hxTarget ?? "#detail-panel")}"`);
...
export function pagination(offset: number, limit: number, total: number, baseUrl: string): string {
  ...
  return `<div class="pagination">
    ...
    <div class="pagination-btns">
      ${hasPrev
        ? `<a class="btn btn-ghost btn-sm" hx-get="${escapeHtml(baseUrl)}&offset=${prevOffset}&limit=${limit}" hx-target="#results" hx-swap="innerHTML">Prev</a>`
      ...
      ${hasNext
        ? `<a class="btn btn-ghost btn-sm" hx-get="${escapeHtml(baseUrl)}&offset=${nextOffset}&limit=${limit}" hx-target="#results" hx-swap="innerHTML">Next</a>`
