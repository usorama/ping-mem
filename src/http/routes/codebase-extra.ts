...
export function registerCodebaseExtraRoutes(app: Hono<AppEnv>, deps: CodebaseExtraRoutesDeps): void {
  ...
  app.delete("/api/v1/codebase/projects/:id", async (c) => {
    try {
      ...
      const mp = path.join(normalized, ".ping-mem", "manifest.json"); if (fs.existsSync(mp)) try { fs.unlinkSync(mp); } catch (e) { warnings.push(`Manifest delete failed: ${e instanceof Error ? e.message : String(e)}`); log.warn("Project delete: manifest cleanup failed", { projectId, error: e instanceof Error ? e.message : String(e) }); }
